...
function convertTimezone(time_info) {
  split_time = time_info.split(':');
  ...
  if(hour < 3){
    if(hour == 2) { con_hr = 12; }
    else if (hour == 1) {con_hr = 11;}
  } else{
    con_hr = hour - 2;
  }
  return con_hr + ":" + ("0"+con_min).substr(-2);
}

function newconvertTimezone(time_info, offset_in_sec) {
  split_time = time_info.split(':');
  hour = parseInt(split_time[0]);
  min = parseInt(split_time[1]);

  const hour_in_sec = hour * 3600;
  const min_in_sec = min * 60;
  const total_in_sec = hour_in_sec + min_in_sec;
  var conv_seconds = total_in_sec + offset_in_sec;
  if (conv_seconds < 0){
      //12 * 3600
      conv_seconds = 43200 + conv_seconds;
  }
  const fraction = conv_seconds / 3600;
  const conv_hr = Math.floor(fraction);
  const conv_min = (fraction - conv_hr) * 60;
  return ("0"+conv_hr) + ":" + ("0"+conv_min).substr(-2);
}
...
function draw() {
  ...
  var dateStr = require("locale").dow(date).toUpperCase().slice(0,3) + " , " + require("locale").date(date).toUpperCase().slice(0,5);
  var dowStr = newconvertTimezone(timeStr, -9000);
  // draw time
  ...
  // draw the different time
  g.setFontAlign(0,0,3).setFont("6x8",2);
